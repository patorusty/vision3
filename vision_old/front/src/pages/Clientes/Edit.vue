  ...
  methods: {
    ...
    buscarCuit: debounce(function() {
      if (this.cliente.nro_dni && this.cliente.cuit.length == 11) {
        http.search('cliente/busquedaCuit?q=' + this.cliente.cuit).then(r => {
          ...
          if (this.cuit.length > 0 && this.cuit != this.cuitViejo) {
            this.cuitUsed = true;
          } else {
            this.cuitUsed = false;
          }
      ...
      if (this.cliente.nro_dni && this.cliente.nro_dni.length >= 6) {
        http.search('cliente/busquedaDNI?q=' + this.cliente.nro_dni).then(r => {
          if (r.data.data.length > 0) {
            this.dni = r.data.data[0].nro_dni;
            if (this.dni && this.dni != this.dniViejo) {
              this.dniUsed = true;
            } else {
              this.dniUsed = false;
            }
    ...
    actualizar() {
      if (
        ...
        http.update('clientes', this.cliente.id, this.cliente).then(() => {
          EventBus.$emit('resetInput', false);
          this.dniViejo = this.cliente.dni;
        });
      }
    },
    cargar() {
      http.loadOne('clientes', this.$route.params.id).then(r => {
        this.cliente = r.data.data;
        this.dniViejo = this.cliente.nro_dni;
        this.cuitViejo = this.cliente.cuit;
      });
    }
  },
  created() {
